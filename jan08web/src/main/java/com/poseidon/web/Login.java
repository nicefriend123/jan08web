...
public class Login extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//jsp에서 name pw 값이 날라옴
//		String id = request.getParameter("id");
//		String pw = request.getParameter("pw"); //오는 값 변수에 저장 , if문 안으로 들어감
		//id : poseidon pw : 0123456789
		if (request.getParameter("id") != null && request.getParameter("pw") !=null) {
			...
			if (dto.getCount() == 1) {
				...
				//세션만들기
				HttpSession session = request.getSession();
				session.setAttribute("mname", dto.getMname()); //mname이라는 이름으로 세션만듬
...
//		System.out.println("id :" + id + " pw :" + pw); //입력 값 확인용
